...
// the numbers 1 and 2. You can rewrite the expression Set(2) + 1 as Set(2).+(1);
// you are calling the method called + defined in the class Set.
// The second expression does not compile. The expression 1 + Set(2) corresponds to 1.+(Set(2));
// you are calling the method + in the class Int. For this reason, the compiler thinks
// you are trying to add the set to the number 1, which is an illegal operation.
