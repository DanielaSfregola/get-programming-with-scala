...
// 1.	The function opsA is total because it always terminates and returns an integer
//    for every integer passed as its parameter.
// 2.	The function opsB is not total: it calls itself recursively and never terminates
//    for positive integers.
// 3.	The function selectException is total because it returns an instance of exception:
//    it computes a value that matches its return type for every input. The keyword throw is
//    missing, so the function does not throw the exception, but it returns it as a class instance.
// 4.	The function anotherToString is total: for every input, it eventually terminates
//    after sleeping for 1 second (or 1000 milliseconds) and returning a string.
